...
func _initialize() -> void:
	...
	# Rogue base stats (rogue_starter.tres): crit_chance=0.15, crit_multiplier=2.0,
	...
	print("Bandit Blade: phys_mult=%.2f crit=%.2f bonus_phys=%.2f (expect 1.20, 0.35, 10.00)" % [
		bandit_stats.physical_damage_multiplier, bandit_stats.crit_chance, bandit_stats.bonus_physical_damage
	])
	assert(is_equal_approx(bandit_stats.physical_damage_multiplier, 1.2))
	assert(is_equal_approx(bandit_stats.crit_chance, 0.35))
	...
	print("Umbral Stiletto: crit=%.2f crit_mult=%.2f (expect 0.25, 3.00)" % [
		umbral_stats.crit_chance, umbral_stats.crit_multiplier
	])
	assert(is_equal_approx(umbral_stats.crit_chance, 0.25))
	...
	print("Bejeweled Push Dagger: phys_mult=%.2f crit=%.2f min_cast_proc=%.2f (expect 1.20, 0.55, 0.20)" % [
		bejeweled_stats.physical_damage_multiplier, bejeweled_stats.crit_chance, bejeweled_stats.min_cast_time_proc_chance
	])
	assert(is_equal_approx(bejeweled_stats.physical_damage_multiplier, 1.2))
	assert(is_equal_approx(bejeweled_stats.crit_chance, 0.55))
